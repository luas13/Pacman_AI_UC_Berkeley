...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for a in self.getLegalActions(state):
            if maxv < self.getQValue(state, a):
    ...
    def computeActionFromQValues(self, state):
        ...
        for a in self.getLegalActions(state):
            if self.getQValue(state, a) == bestQ:
                actions.append(a)
        # Break ties randomly, pick randomly
    ...
    def update(self, state, action, nextState, reward):
        ...
        finalv = (1-alpha)*oldv + alpha*(reward + discount* v)
        self.values[(state, action)] = finalv
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for f, v in features.iteritems():
            self.weights[f] += self.alpha * (newv - oldv) * features[f]
